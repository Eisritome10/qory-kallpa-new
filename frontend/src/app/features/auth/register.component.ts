      ...
      @if (registeredEmail(); as email) {
        <div class="card text-center">
          ...
          <p class="mt-2 text-sm text-marino-500">
            Enviamos un enlace de verificacion a <span class="font-semibold text-marino-800">{{ email }}</span>.
          ...
          <button type="button" class="btn-secondary mt-6 w-full" [disabled]="resending() || resent()" (click)="onResend(email)">
            {{ resent() ? 'Correo reenviado' : resending() ? 'Reenviando...' : 'No recibi el correo, reenviar' }}
          </button>

          <p class="mt-6 text-center text-sm text-marino-500">
            <a routerLink="/ingresar" class="font-semibold text-naranja-600 hover:underline">Ir a iniciar sesion</a>
          </p>
        </div>
      } @else {
        <div class="card">
          <h1 class="text-2xl font-extrabold text-marino-900">Crea tu cuenta de postulante</h1>
          <p class="mt-1 text-sm text-marino-500">Necesitas una cuenta para enviar tu postulacion como voluntario/a.</p>
          ...
          <form class="mt-6 space-y-4" [formGroup]="form" (ngSubmit)="onSubmit()">
            <div>
              ...
              @if (isInvalid('fullName')) {
                <p class="field-error">Ingresa tu nombre completo (minimo 3 caracteres).</p>
              }
            </div>
            ...
            <div>
              ...
              @if (isInvalid('password')) {
                <p class="field-error">La contrasena debe tener al menos 8 caracteres.</p>
              }
            </div>

            <div>
              <label class="mb-1 block text-sm font-medium text-marino-800">Confirma tu contrasena</label>
              ...
              @if (form.errors?.['passwordsMismatch'] && form.get('confirmPassword')?.touched) {
                <p class="field-error">Las contrasenas no coinciden.</p>
              }
            </div>
          ...
          <p class="mt-6 text-center text-sm text-marino-500">
            ¿Ya tienes cuenta?
            <a routerLink="/ingresar" class="font-semibold text-naranja-600 hover:underline">Ingresa aqui</a>
          </p>
        </div>
      }
    </section>
  ...
  onSubmit(): void {
    ...
    this.authService.register({ fullName, email, password }).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.loading.set(false);
        this.errorMessage.set(error.error?.message ?? 'No se pudo crear la cuenta. Intentalo nuevamente.');
      },
    });
  }
